...
int main(int argc, char* argv[])
{
	...
	std::vector<std::vector<double>> slab1(numAtomsPerSlab, std::vector<double>(3));
		...
		for ( int m=0; m<unitCellGrid[1]; ++m ) // cells along y
		{
			...
			for ( int n=0; n<unitCellGrid[2]; ++n ) // cells along z
			{
				...
				for ( int i=0; i<numAtomsPerUnitCell; ++i )
				{
					for ( int j=0; j<3; ++j )
					{
						slab1[atomCounter][j] = unitCell[i][j] + offset[j];
					}
					++atomCounter;
				}
			}
	...
	//----- Write a GRO file with the coordinates -----//
	...
	/* Include atom type and LJ information here for posterity/convenience/reference
	// Lennard-Jones settings
	ofs << "[ defaults ]" << "\n";
    ofs << "; nbfunc     comb-rule      gen-pairs     fudgeLJ      fudgeQQ" << "\n";
    ofs << "  1          2              yes           0.5          0.833333" << "\n";
    ofs << "\n";

	// Atom types (include TIP4P/Ice atoms for the heck of it)
    ofs << "[ atomtypes ]" << "\n";
    ofs << "; type    mass       charge      ptype     sigma[nm]         epsilon[kJ/mol]" << "\n";
    ofs << "  OW      16.000      0.0000     A         0.31668e+00       0.8821154e+00" << "\n";
    ofs << "  HW       1.008     +0.5897     A         0.0000e+00        0.0000e+00" << "\n";
    ofs << "  MW       0.000     -1.1794     V         0.0000e+00        0.0000e+00" << "\n";
    ofs << "  AlOK    26.982      1.575      A	       4.2712E-01        5.563050e-06" << "\n";
    ofs << "  SiTK    28.086      2.1        A	       3.3020E-01        7.700650e-06" << "\n";
    ofs << "  OBK     16.000     -1.05       A	       3.1655E-01        6.501900e-01" << "\n";
    ofs << "  OHK     16.000     -0.95       A	       3.1655E-01        6.501900e-01" << "\n";
    ofs << "  HHK	   1.008      0.425      A	       0.0000E+00        0.000000e+00" << "\n";
	ofs << "\n";
	*/
	...
	for ( int i=0; i<numUnitCellsPerSlab; ++i )
	{
		...
		for ( int n=0; n<numAtomsPerUnitCell; ++n )
		{
			...
			// Charge group index
			ofs << std::setw(6) << (chargeGroupIndex + 1) << " ";
	...
	// Position restraints for (partially) rigid surfaces

	double k_restraint = 10000; // [kJ/mol*nm^2]
	int    posResType = 1;      // Generate exclusions

	// - Indices (in the unit cell) of the Al surface's O and H atoms
	std::vector<int> unitCellSurfaceO = { 10, 11, 12, 27, 28, 29 };
	std::vector<int> unitCellSurfaceH = { 14, 15, 33, 31, 32, 16 };
	int numSurfaceHydroxylGroups = unitCellSurfaceO.size();

	int topLayer = unitCellGrid[2] - 1;
	bool isRestrained;

	ofs << "[ position_restraints ] " << "\n";
	ofs << "; ai   k_res " << "\n";

	// - Free HH at exposed Al surface (everything else restrained)

	ofs << "#ifdef AL_SURFACE_FREE_H" << "\n";
	...
	for ( int l=0; l<unitCellGrid[0]; ++l )
	{
		for ( int m=0; m<unitCellGrid[1]; ++m )
		{
			...
			{
				// Current unit cell and index of its first atom
				int cellIndex       = l*unitCellGrid[0] + m*unitCellGrid[1] + n;
				int atomIndexOffset = numAtomsPerUnitCell*cellIndex;
				...
				for ( int i=0; i<numAtomsPerUnitCell; ++i )
				{
					...
					if ( l == topLayer )
					{
						for ( int j=0; j<numSurfaceHydroxylGroups; ++j )
						{
							...
							{
								// Atom is a surface H: don't restrain it
								isRestrained = false;
							}
						}
					}
					...
					if ( isRestrained )
					{
						...
						ofs << " " << std::setw(5) << (atomIndex + 1)
						    << " " << std::setw(3) << posResType
						    << " " << std::setw(8) << k_restraint << "\n";
					}
				}
			}
		}
	}
	ofs << "; End #ifdef AL_SURFACE_FREE_H" << "\n";
	ofs << "#endif" << "\n"; 
	ofs << "\n";

	// - Free hydroxyl groups at exposed Al surface (everything else restrained)
	ofs << "#ifdef AL_SURFACE_FREE_OH" << "\n";
				...
				for ( int i=0; i<numAtomsPerUnitCell; ++i )
				{
					...
					if ( l == topLayer )
					{
						for ( int j=0; j<numSurfaceHydroxylGroups; ++j )
						{
							...
							{
								// Atom is a surface O: don't restrain it
								isRestrained = false;
							}
						}
					}
