...
class SimulationBox
{
 public:
	SimulationBox();
	SimulationBox(const double l_x, const double l_y, const double l_z);
	SimulationBox(const Real3& lengths);
	~SimulationBox();
	...
	const Box& get_box_matrix() const {
		return box_matrix_;
	}
...
// Calculate the minimum-image vector between 2 points and its squared magnitude
template <typename T>
void SimulationBox::calculateDistance(const T x1[3], const T x2[3], Real3& x12, 
...
template <typename T>
void SimulationBox::calculateDistance(const T x1[3], const Real3& x2, 
...
template <typename T>
void SimulationBox::calculateDistance(const Real3& x1, const T x2[3],
                             T x12[3], double& dist_sq) const
{
	dist_sq = 0.0;
	for ( int d=0; d<3; ++d ) {
...
template <typename T>
void SimulationBox::calculateDistance(const Real3& x1, const T x2[3], 
                             Real3& x12, double& dist_sq) const
{
	dist_sq = 0.0;
	for ( int d=0; d<3; ++d ) {
		x12[d] = x2[d] - x1[d];
